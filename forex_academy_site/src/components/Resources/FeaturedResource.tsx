...
// };
// src/components/FeaturedResource.tsx
'use client';
...
const FeaturedResource: React.FC<FeaturedResourceProps> = ({
  ...
  const { title, description, image, link, cta = "Explore Now" } = resource;
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`relative rounded-2xl overflow-hidden shadow-[0_0_30px_rgba(0,200,150,0.15)] mb-16 ${className}`}
    >
      {/* Background Image */}
      ...
      <div className="absolute inset-0 flex items-center px-6 sm:px-10 md:px-16">
        <div className="max-w-xl">
          <motion.h3
            ...
            className="text-lg sm:text-3xl font-bold text-[#00c896] leading-tight"
          >
            {title}
          </motion.h3>
          ...
          <motion.a
            ...
            className="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-[#00c896] text-black font-semibold rounded-full 
                       hover:scale-105 hover:shadow-[0_0_25px_rgba(0,200,150,0.5)] transition-all duration-300 text-sm sm:text-base"
          >
